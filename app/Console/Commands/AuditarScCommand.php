...
class AuditarScCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($operacionesSC as $operacion) {
            ...
            $estado = $this->compararMontos((float)$operacion->cargo_edc, (float)$datosSC['monto_impuestos']);
            ...
            $auditoriasParaGuardar[] = [
                ...
                'monto_impuestos_mxn' => $datosSC['monto_impuestos'],
                'estado'             => $estado,
                'updated_at'         => now(),
            ];
        ...
        if (!empty($auditoriasParaGuardar)) {
            ...
            AuditoriaSc::upsert(
                ...
                ['fecha_sc', 'folio_sc', 'ruta_txt', 'ruta_pdf', 'cargo_edc', 'monto_impuestos_mxn', 'monto_impuestos_sc', 'estado'] // Columnas a actualizar
            );
        ...
        try {
            ...
            foreach ($finder as $file) {
                ...
                if (preg_match('/\[encOBSERVACION\][^\d]*(\d{7})/', $contenido, $matchesPedimento)) {
                    ...
                    $indice[$pedimento] = [
                        'monto_impuestos' => isset($matchMonto[1]) ? (float)trim($matchMonto[1]) : 0.0,
