    ...
    public function handle()
    {
        ...
        try {
            ...
            if (strtoupper($tarea->sucursal) === 'ZLO' || strtoupper($tarea->sucursal) === 'MANZANILLO') {
                ...
                // Extraemos los Pagos de Derecho a la BD local PRIMERO, porque el 
                // método de envío los necesita leer de ahí.
                gc_collect_cycles();
                $this->info("Extrayendo Pagos de Derecho de los PDFs a la BD...");
                $status = $controller->auditarFacturasDePagosDeDerecho($tarea->id);
                if ($status['code'] > 0) {
                    throw $status['message'];
                }

                // 10. AHORA SÍ, PAGOS DE DERECHO A GPC
                gc_collect_cycles();
