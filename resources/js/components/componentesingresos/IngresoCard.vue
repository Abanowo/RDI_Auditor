    ...
    <div
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 ml-2">
            <div class="flex items-center gap-4">
                <span class="px-3 py-1 text-white font-black rounded uppercase shadow-sm text-sm"
                    style="background-color: #2A3A4D;">
                    {{ item.sucursal_origen || 'N/A' }}
                </span>
                <span class="text-base font-bold text-gray-500">{{ item.fecha }}</span>
                <span
                    class="text-sm font-bold text-indigo-700 bg-indigo-100 border border-indigo-200 px-3 py-1 rounded uppercase hidden sm:block">
                    Ref: {{ item.folio_sc || 'N/A' }}
                </span>
            </div>
            <div class="flex items-center gap-3">
                <span class="font-bold text-gray-400 uppercase text-sm hidden md:block">Banco:</span>
                <span class="text-base font-bold text-gray-700">{{ item.banco_receptor || 'N/E' }}</span>
            </div>
        </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-between items-center ml-2">
            <div class="flex items-center gap-5">
                <span v-if="item.tipo_comprobante === 'CFDI'"
                    class="px-4 py-1.5 bg-blue-100 text-blue-700 font-bold rounded border border-blue-200 text-sm">CFDI</span>
                <span v-else-if="item.tipo_comprobante === 'Nota Cargo'"
                    class="px-4 py-1.5 bg-purple-100 text-purple-700 font-bold rounded border border-purple-200 text-sm">NOTA
                    CARGO</span>
                <span v-else
                    class="px-4 py-1.5 bg-gray-200 text-gray-600 font-bold rounded border border-gray-300 text-sm">{{
                        item.tipo_comprobante || 'N/A' }}</span>

                <div class="h-5 w-px bg-gray-300"></div>
                <span class="text-base font-bold text-gray-500">Comp: <span class="text-gray-800">{{
                    item.folio_complemento || 'SIN COMPLEMENTO' }}</span></span>

                <div class="h-5 w-px bg-gray-300"></div>
                <span class="font-bold px-4 py-1.5 rounded uppercase border text-sm"
            ...
            <div class="flex items-center gap-2.5">
                <button @click="$emit('generar', item)"
                    class="text-green-600 hover:text-white bg-green-100 hover:bg-green-500 p-2.5 rounded-lg transition-colors"
                ...
                <button @click="$emit('visualizar', item)"
                    class="text-blue-600 hover:text-white bg-blue-100 hover:bg-blue-500 p-2.5 rounded-lg transition-colors"
                ...
                <button @click="$emit('enviar', item)"
                    class="text-purple-600 hover:text-white bg-purple-100 hover:bg-purple-500 p-2.5 rounded-lg transition-colors"
                ...
                <button @click="$emit('editar', item)"
                    class="text-indigo-600 hover:text-white bg-indigo-100 hover:bg-indigo-500 p-2.5 rounded-lg transition-colors"
                    title="Editar Fila">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="$emit('eliminar', item.id)"
                    class="text-red-500 hover:text-white bg-red-100 hover:bg-red-500 p-2.5 rounded-lg transition-colors"
                    title="Eliminar Fila">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
